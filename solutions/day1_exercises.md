...
Let _x_ be a numeric matrix of size 10000 × 10000. Try adding random
numbers to:

1.  100 randomly chosen **rows** of _x_,
2.  100 randomly chosen **columns** of _x_.
...
Repeat the exercise, but with _x_ being a data frame instead of a
matrix.
...
Write a function `powers(a, b)` that for two vectors _a_, *b* calculates
a matrix _C_, with
_c_<sub>_i\*\*j_</sub> = *a*<sub>_i_</sub><sup>_b_<sub>_j_</sub></sup>.
...
_Hint:_ use the `radix()` function to convert single numbers to results
...
The Central Limit Theorem says that a standardized mean of _n_
independent identically distributed random variables approaches the
standard Normal distribution with increasing _n_. Let’s test it!

Define a function `sample_dice(n, m)` that does the following:

- generate _n_ experiments of _m_ dice rolls each,
